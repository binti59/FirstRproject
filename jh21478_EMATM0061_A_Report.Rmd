...
#Using the Total law of probability and Bye's theorem -we derive the value of phi returned by the function below
...
```{r}
#d) Give an expression for the population variance of the random variable X in terms of α,β,γ,n
```     
    \[ Var(\overline X) =E[\overline X^2]- E[\overline X]^2  \\
    =E[\frac{\sum_{i=1}^{n}X_i^2}n]- E[\frac{\sum_{i=1}^{n}X_i}n]^2  \\
  =(\alpha+4\beta+25\gamma) -\frac{n(\alpha+2\beta+5\gamma)^2}{n^2} \\
  =\alpha+4\beta+25\gamma-\frac{ (\alpha^2+4\beta^2+25\gamma^2+4\alpha\beta+20\beta\gamma+10\alpha\gamma)}n
 \] 
```{r}
#Create a function called sample_X_0125() which takes as inputs α, β, γ and n and outputs a sample X1, . . . , Xn of independent copies of X where P (X = 1) = α, P (X = 2) = β, P (X = 5)= γ and P (X /∈ {0, 1, 2, 5}) = 0
...
{
  glmRidge=glmnet(x=train_x,y=train_y ,alpha=0,lambda=lambda) #trainmodel
